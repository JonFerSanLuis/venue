    ...
    <div class="py-12 bg-gray-50 min-h-screen">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-8 flex justify-between items-end">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 uppercase tracking-tight">Crear Nuevo Festival</h1>
                    <p class="text-sm text-gray-500 mt-1">Registra los datos oficiales del evento en la base de datos.</p>
                </div>
            ...
            <form action="{{ route('festivals.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="p-8 space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="md:col-span-2">
                                ...
                                <input type="text" name="name" class="w-full border-gray-300 py-3 px-4 rounded-sm text-sm focus:ring-gray-900 focus:border-gray-900" placeholder="Ej: Mad Cool Festival" required>
                            </div>
                            <div>
                                <label class="block text-xs font-bold uppercase text-gray-600 mb-2">Ciudad / Recinto</label>
                                <input type="text" name="location" class="w-full border-gray-300 py-3 px-4 rounded-sm text-sm focus:ring-gray-900 focus:border-gray-900" placeholder="Ej: Madrid, Espacio Iberdrola" required>
                            </div>
                            <div>
                                <label class="block text-xs font-bold uppercase text-gray-600 mb-2">Estilo Principal</label>
                                <input type="text" name="style" class="w-full border-gray-300 py-3 px-4 rounded-sm text-sm focus:ring-gray-900 focus:border-gray-900" placeholder="Ej: Indie / Rock" required>
                            </div>
                            <div>
                                <label class="block text-xs font-bold uppercase text-gray-600 mb-2">Fecha del Evento</label>
                                <input type="date" name="start_date" class="w-full border-gray-300 py-3 px-4 rounded-sm text-sm focus:ring-gray-900 focus:border-gray-900" required>
                            </div>
                            <div>
                                <label class="block text-xs font-bold uppercase text-gray-600 mb-2">Cartel del Festival (Imagen)</label>
                                <input type="file" name="image" accept="image/*" class="w-full border-gray-300 py-2 px-4 rounded-sm text-sm file:mr-4 file:py-2 file:px-4 file:border-0 file:text-xs file:font-bold file:uppercase file:bg-gray-900 file:text-white hover:file:bg-black cursor-pointer" required>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white border border-gray-200 rounded-sm shadow-sm overflow-hidden mb-6">
                    ...
                    <div class="p-6">
                        <div id="ticket-types-container" class="space-y-4">
                            ...
                            <div class="ticket-row grid grid-cols-12 gap-3 items-end p-4 bg-gray-50 border border-gray-200 rounded-sm">
                                <div class="col-span-5">
                                    ...
                                    <input type="text" name="ticket_names[]" placeholder="Ej: General, VIP..." class="w-full border-gray-300 py-2 px-3 rounded-sm text-sm focus:ring-gray-900 focus:border-gray-900" required>
                                </div>
                        ...
                        <p class="text-xs text-gray-400 mt-3">Añade al menos un tipo de entrada (General, VIP, Early Bird, etc.)</p>
                    </div>
                </div>
    ...
    <script>
        document.getElementById('add-ticket').addEventListener('click', function() {
            ...
            row.innerHTML = `
                <div class="col-span-5">
                    <label class="block text-xs font-bold uppercase text-gray-600 mb-2">Nombre</label>
                    <input type="text" name="ticket_names[]" placeholder="Ej: VIP, Early Bird..." class="w-full border-gray-300 py-2 px-3 rounded-sm text-sm focus:ring-gray-900 focus:border-gray-900" required>
                </div>
                <div class="col-span-3">
                    <label class="block text-xs font-bold uppercase text-gray-600 mb-2">Precio (€)</label>
                    <input type="number" name="ticket_prices[]" step="0.01" min="0" placeholder="0.00" class="w-full border-gray-300 py-2 px-3 rounded-sm text-sm focus:ring-gray-900 focus:border-gray-900" required>
