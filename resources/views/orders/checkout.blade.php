...
<body class="bg-black text-white font-sans antialiased selection:bg-pink-500 selection:text-white">
                    ...
                    <form action="{{ route('orders.store', [$festival->id, $ticketType->id]) }}" method="POST" class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="buyer_name" value="{{ old('buyer_name', Auth::user()->name) }}"
                                class="w-full bg-gray-950 border border-gray-700 text-white px-4 py-3 text-sm focus:border-pink-500 focus:outline-none transition-colors" required>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="tel" name="buyer_phone" value="{{ old('buyer_phone') }}"
                                class="w-full bg-gray-950 border border-gray-700 text-white px-4 py-3 text-sm focus:border-pink-500 focus:outline-none transition-colors" placeholder="+34 600 000 000">
                        </div>
                        ...
                        <div class="bg-yellow-900/20 border border-yellow-500/20 p-4">
                            <p class="text-yellow-500 text-xs font-bold uppercase tracking-wide">⚠ Plataforma de demostración — no se realizará ningún cargo real.</p>
                        </div>
    ...
    <script>
        ...
        select.addEventListener('change', function() {
            total.textContent = (price * parseInt(this.value)).toFixed(2).replace('.', ',') + '€';
        });
    </script>
